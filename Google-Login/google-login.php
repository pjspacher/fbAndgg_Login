...
if ($gClient->getAccessToken()) {
    ...
    if ($conn->connect_error) {
            ...
            $sql = "SELECT * FROM u13570178 WHERE id_user = ".$gg_id."";
            $result = $conn->query($sql);

            if ($result -> num_rows == 0){
                $sql_insert = "INSERT INTO u13570178(id_user, first_name, last_name, email, link, gender, locale, fb_createtime) VALUES ('".$gg_id."', '".$gg_fname."', '".$gg_lname."', '".$gg_email."', '".$gg_link."', '".$gg_gender."', '".$gg_local."',Now())";

                $conn->query($sql_insert);
                header('Location:'.filter_var($gRedirectURL,FILTER_SANITIZE_URL));
            }else{
                echo "<h3 style='color:#3dcec5';>Login Success</h3>";
                echo "<br>";
                echo "<ul>";
                    ...
                    while ($row = $result -> fetch_assoc()) {
                        ...
                        echo "<br>";
                        echo '<li><a class="btn btn-block btn-danger btn-lg" href="logout.php" style="margin-top:0;">Log out</a></li>';
                    }
                echo "</ul>";
                $conn->close();
        }
    ...
    echo '<a class="btn btn-block btn-social btn-lg btn-google" href="'.$gloginUrl.'"><span class="fa fa-google"></span> Sign in with Google</a>';
